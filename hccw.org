...
~S-<right>~ and ~S-<left>~ cycles through to-do states for a heading, i.e. from /draft/ to /revise/ to /done/.
...
The =Vale= syntax checker will not check inside headings tagged with /noexport/. Outside the /noexport/ sections we can use these comments to turn Vale off and on:
#+begin_src
...
# LTeX: enabled=true
Use =C-q SPACE= to insert a literal space after an abbreviation instead of expanding it.
