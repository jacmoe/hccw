...
-   `* todo heading` a heading with a to-do
...
`S-<right>` and `S-<left>` cycles through to-do states for a heading, i.e. from _draft_ to _revise_ to _done_.
...
-   `- [ ] list item` list item with unchecked checkbox
-   `- [X] list item` list item with checked checkbox
